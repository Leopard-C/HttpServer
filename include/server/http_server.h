...
public:
    ...
    /**
     * @brief 设置回调函数：工作线程(创建完成后)开始运行前执行.
     */
    void set_cb_before_worker_thread_run(std::function<void()> cb) { cb_before_worker_thread_run_ = cb; }
...
private:
    ...
    std::function<void()> cb_before_worker_thread_run_;
